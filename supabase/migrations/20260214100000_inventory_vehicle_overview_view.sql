...
begin
  if to_regclass('public.vehicles') is null then
    raise notice 'Skipping inventory_vehicle_overview view: public.vehicles does not exist yet';
    return;
  end if;
  ...
  execute 'grant select on public.inventory_vehicle_overview to authenticated';
end;
$$;
